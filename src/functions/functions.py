...
def calc_rot_angle(x0: float, y0: float, x1: float, y1: float) -> tuple[float, float]:
    """
    ...
    ----------
    alpha_x: float
        Angle of rotation to switch from global to local reference frame (from x_glob to x_loc)
...
def caluclate_shifts(roll_deg: float, pitch_deg:float, height: float = 1500) -> tuple[float, float]:
    """
    Calculates values and directions of shifts along the axes of vehicle local reference frame for tilt compensation.
    
    Parameters
    ----------
    roll_deg: float
        Tilt along y-axis of the vehicle local reference frame in degrees.
        Positive roll corresponds to the orientation when the right side of the vehicle is being lower than the left side.
        
    pitch_deg: float
        Tilt along x-axis of the vehicle local reference frame in degrees.
        Positive pitch corresponds to the orientation when the front part of the vehicle is being lower than the rear part.
...
def apply_shifts(x: float, y: float,
                 alpha_x: float, alpha_y: float,
                 shift_x: float = 0, shift_y: float = 0) -> tuple[float, float]:
    """
    Shifts point of xy-plane along local reference frame of the vehicle for tils compensation
...
def add_zero_element(df: pd.DataFrame, dataset: DataSet, i: int) -> None:
    """
    ...
    """
    curr_row = df.iloc[i]
    shift_x, shift_y = caluclate_shifts(curr_row['roll_deg'], curr_row['pitch_deg'])
    dataset.add_data(*curr_row[:5], shift=(shift_x, shift_y))

def add_first_element(df: pd.DataFrame, dataset: DataSet, i: int) -> None:
    """
    ...
    alpha_x, alpha_y = calc_rot_angle(prev_row.x_mm, prev_row.y_mm,
                                            curr_row.x_mm, curr_row.y_mm)
    
    # Calculate the shift for current row
    shift_x, shift_y = caluclate_shifts(curr_row.roll_deg, curr_row.pitch_deg)
    # Apply shift to current row
    x_adj, y_adj = apply_shifts(curr_row.x_mm, curr_row.y_mm, alpha_x, alpha_y,
                    shift_x, shift_y)
    # Record the results for current row
    dataset.add_data(*curr_row[:5], angle=(alpha_x, alpha_y),
                        shift=(shift_x, shift_y), adj_x=x_adj, adj_y=y_adj)
    
    # Apply shift to previous row
    x_adj_prev, y_adj_prev = apply_shifts(prev_row.x_mm, prev_row.y_mm, alpha_x, alpha_y,
                    dataset.shift[i-1][0], dataset.shift[i-1][1])
...
def recalc_prev_elements(df: pd.DataFrame, dataset: DataSet,
                         correct_alpha_x: float, correct_alpha_y: float, i: int) -> None:
    """
    ...
    correct_alpha_x: float
        Correct angle of rotation for x-axis between two points with the same roll
        
    correct_alpha_y: float
        Correct angle of rotation for y-axis between two points with the same roll
    ...
    for j in reversed(range(1, i)):
        ...
        x_curr_adj, y_curr_adj = apply_shifts(dataset.adj_x[j], dataset.adj_y[j],
    ...
    x_adj, y_adj = apply_shifts(dataset.x_mm[0], dataset.y_mm[0], dataset.angle[0][0], dataset.angle[0][1],
                dataset.shift[0][0], dataset.shift[0][1])
    # Record adjusted points
    dataset.adj_x[0], dataset.adj_y[0] = x_adj, y_adj
    

def correct_point(df: pd.DataFrame, dataset: DataSet, i: int, flag: bool, recalc=True) -> bool:
    """
    Calculates the angle between previous point (adjusted to the same roll as current) and current point
    Corrects coordintes of the current point
    If two points with same roll_deg are encountered, calculates the correct position of local reference frame
    ...
    flag: bool
        flag = True if the points with the same roll have not yet been encountered, False otherwise
    
    recalc: bool, default = True
        recalculate previous points' coordinates after encountering points with the same roll
    ...
    if (curr_row.roll_deg == prev_row.roll_deg) & (flag):
        ...
        x_prev_adj, y_prev_adj = apply_shifts(dataset.adj_x[i-1], dataset.adj_y[i-1],
                                                alpha_x=dataset.angle[i-1][0],
                                                alpha_y=dataset.angle[i-1][1],
                                                shift_y=y_shift)
        # Calculate angle
        alpha_x, alpha_y = calc_rot_angle(x_prev_adj, y_prev_adj,
                                            curr_row.x_mm, curr_row.y_mm)
...
def transfrom(df, dataset, recalc = True):
    """
