...
def EDA(df: pd.DataFrame) -> None:
    ...
    # Position
    ax[0][0].scatter(df.x_mm, df.y_mm, s=20, c='orangered')
    ax[0][0].plot(df.iloc[[0, -1]]['x_mm'], df.iloc[[0, -1]]['y_mm'], linewidth=1)
    ...
    ax[0][1].hlines(df.timedelta.mean(), xmin=0, xmax=df.shape[0], linestyles='dashed', colors=['orangered'])
    ...
    ax[1][0].hlines(df.roll_deg.mean(), xmin=0, xmax=df.shape[0], linestyles='dashed', colors=['orangered'])
    ...
    ax[1][1].hlines(df.pitch_deg.mean(), xmin=0, xmax=df.shape[0], linestyles='dashed', colors=['orangered'])
...
def post_plot_1(df_new: pd.DataFrame) -> None:
    ...
    # ax 1
    ax[0].scatter(df_new.x_mm, df_new.y_mm, s=10)
    ax[0].scatter(df_new.adj_x, df_new.adj_y, c='r', s=10)

    for i in range(len(df_new.time_s)):
        #plt.plot([dataset.x_mm[i], dataset.adj_x[i]], [dataset.y_mm[i], dataset.adj_y[i]], c='black')
        ax[0].arrow(df_new.x_mm[i], df_new.y_mm[i], (df_new.adj_x[i]-df_new.x_mm[i]), (df_new.adj_y[i]-df_new.y_mm[i]),
                width = 10, head_width=30)
        
    for i in range(len(df_new)):
        ax[0].text(df_new.iloc[i]['x_mm'] + 30, df_new.iloc[i]['y_mm'], f"pitch: {df_new.iloc[i]['pitch_deg']}, roll: {df_new.iloc[i]['roll_deg']}", fontsize=7)
    ...
    # ax 1
    ax[1].scatter(df_new.x_mm, df_new.y_mm, s=30)
    ax[1].scatter(df_new.adj_x, df_new.adj_y, c='r', s=30)
    ax[1].plot(df_new.iloc[[0, -1]].x_mm, df_new.iloc[[0, -1]].y_mm, c='orangered', linewidth=1)
    ax[1].plot(df_new.iloc[[0, -1]].adj_x, df_new.iloc[[0, -1]].adj_y, c='royalblue', linewidth=1)

    for i in range(len(df_new.time_s)):
        #plt.plot([dataset.x_mm[i], dataset.adj_x[i]], [dataset.y_mm[i], dataset.adj_y[i]], c='black')
        ax[1].arrow(df_new.x_mm[i], df_new.y_mm[i], (df_new.adj_x[i]-df_new.x_mm[i]), (df_new.adj_y[i]-df_new.y_mm[i]),
...
def post_plot_2(df_new: pd.DataFrame, angles: list[float]) -> None:
    ...
    for i in range(len(headings)):
        ...
        ax[0].arrow(x_start, y_start, x_end-x_start, y_end-y_start, head_width=50, head_length=50, width=10, fc='royalblue', ec='royalblue')
